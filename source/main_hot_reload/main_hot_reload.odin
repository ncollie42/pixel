...
copy_dll :: proc(to: string) -> bool {
	err := os.copy_file(to, GAME_DLL_PATH)
	return err == nil
}
...
load_game_api :: proc(api_version: int) -> (api: Game_API, ok: bool) {
	mod_time, mod_time_error := os.modification_time_by_path(GAME_DLL_PATH)
	if mod_time_error != nil {
		fmt.printfln("Failed getting last write time of " + GAME_DLL_PATH + ", error code: {1}", mod_time_error)
		return
	}
...
frame :: proc "c" () {
	...
	if game_dll_mod_err == nil && game_api.modification_time != game_dll_mod {
		reload = true
	}
